...
if (req.file)   {
    ...
    const updatepic=await cloudinary.uploader.upload(req.file.path ,{
        folder:'profiles',
        resource_type:'image',
        overwrite:true,
    })
...
const removeBio=catchAsync(async(req,res)=>{
    const id=req.params.id;
    const profile=await Profile.findByIdAndUpdate(id,{$unset:{bio:1}},{new:true})
    if(!profile){
        res.status(400).json({  
            status:'fail',
            message:'Profile ID is not found'
...
const removePhoto=catchAsync(async(req,res)=>{
        const id=req.params.id;
            await cloudinary.uploader.destroy(profile.avatarId);
    const profile=await Profile.findByIdAndUpdate(id,{$unset:{avatarId:1,avatarUrl:1}},{new:true})
    if(!profile){
        res.status(400).json({  
            status:'fail',
            message:'Profile ID is not found'
